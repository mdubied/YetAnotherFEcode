...
function Lr = reduced_cost_function_w_constraints(N,tensors_hydro_PROM,eta,etad,xiRebuild,xi,dr,A,b,barrierParam)
    Lr = 0;
    nConstraints = size(b);
    ...
    for t=1:N-2
        ...
        double(tensors_hydro_PROM.Tr2*xiRebuild) + ...
        double(tensors_hydro_PROM.Tru2*eta_i) + double(tensors_hydro_PROM.Trudot2*etad_i) + ...
        double(ttv(ttv(tensors_hydro_PROM.Tru3,xiRebuild,3),eta_i,2)) + double(ttv(ttv(tensors_hydro_PROM.Trudot3,xiRebuild,3),etad_i,2)) + ...
