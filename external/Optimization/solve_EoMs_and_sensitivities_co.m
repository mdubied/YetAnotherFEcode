% solve_EoMs_and_sensitivities_co
%
% Synthax:
% TI_NL_PROM = solve_EoMs_and_sensitivities_co(V,ROM_Assembly,tailProperties,spineProperties,actuTop,actuBottom,h,tmax)
%
...
% (10) pActu:               current value of the actuation parameters
%
% OUTPUTS:
...
function TI_NL_PROM = solve_EoMs_and_sensitivities_co(V,PROM_Assembly,fIntTensors,tailProperties,spineProperties,dragProperties,actuTop,actuBottom,h,tmax,pActu)
    ...
    U = PROM_Assembly.U;
    nParam = size(U,2)+length(pActu);   % shape param. + actu. param.
    s0 = zeros(size(V,2),nParam);
    sd0 = zeros(size(V,2),nParam);
    sdd0 = zeros(size(V,2),nParam);
    ...
    % modal nonlinear Residual evaluation function handle (EoMs)
    ...
    % residual function handle (sensitivity)
    Residual_sens = @(s,sd,sdd,etaSol,etadSol,etaddSol,drdqdd, drdqd, ...
        drdq,t)residual_linear_sens_combined_co(s,sd,sdd,t, ...
        etaSol,etadSol,etaddSol, drdqdd, drdqd, drdq,...
        pd_fint,pd_tail,pd_spine,pd_drag,pd_shape_actuTop,pd_shape_actuBottom, ...
        actuSignalT,actuSignalB,pd_actu_actu,pActu);
