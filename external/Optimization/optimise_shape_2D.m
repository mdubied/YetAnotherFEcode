...
function [xiStar,xiEvo,LrEvo] = optimise_shape_2D(myElementConstructor,nset,nodes,elements,U,d,h,tmax,A,b,varargin)
    ...
    TI_NL_PROM = solve_EoMs(V,PROM_Assembly,tailProperties,spineProperties,actuTop,actuBottom,h,tmax);
    toc
    ...
    eta0 = V.'*reshape(nodes.',[],1);     % initial node position expressed in the ROM
    ...
    Lr = reduced_cost_function_w_constraints(N,tailProperties,spineProperties,eta0,eta,etad,etadd,xiRebuild_k,xi_k,dr,A,b,barrierParam);  
    LrEvo = Lr;
    ...
    for k = 1:maxIteration
        ...
        fprintf('____________________\n')
        fprintf('Computing gradient of the cost function...\n') 
        nablaLr = gradient_cost_function_w_constraints(dr,xiRebuild_k,xi_k,eta0,eta_k,etad_k,etadd_k,S,Sd,Sdd,tailProperties,spineProperties,A,b,barrierParam);
        LrEvo = [LrEvo, reduced_cost_function_w_constraints(N,tailProperties,spineProperties,eta0,eta_k,etad_k,etadd_k,xiRebuild_k,xi_k,dr,A,b,barrierParam)];
